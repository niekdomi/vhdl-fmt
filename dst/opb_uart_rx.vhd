...
end entity OPB_uart_RX;

library UNISIM;
use UNISIM.all;

architecture IMP of OPB_uart_RX is
  ...
  signal com_rx_Nr_of_Data       : std_logic_vector(3 downto 0);
  signal com_Int_nr_of_Data      : std_logic_vector(3 downto 0);
  ...
  constant SERIAL_TO_PAR_LENGTH : integer := 1 + C_DATA_BITS;--Calc_Length;
 ...
 -- constant C_INT_NR_OF_D        : std_logic_Vector(3 downto 0) := "1101";       -- 13    
  --constant C_TIMEOUT            : std_logic_vector(7 downto 0) := "10100000";   -- 160
    
  signal FIFO_Write    : std_logic;   
  signal data	       : std_logic_vector(0 to SERIAL_TO_PAR_LENGTH);
  signal fifo_Din      : std_logic_vector(0 to C_DATA_BITS-1);
  ...
  signal buffer_Int : std_logic;
  signal time_out    : std_logic;
  signal TimeOutCnt : std_logic_vector(8 downto 0);    
  signal Data_Present :std_logic;
  
begin  -- architecture IMP

    o_RX_nr_of_Data <= rx_Nr_of_Data; 
	RX_Data_Present <= Data_Present;  
	process(rx_Nr_of_Data,i_Int_nr_of_Data)
	begin
		for i in rx_Nr_of_Data'range loop
			com_rx_Nr_of_Data(com_rx_Nr_of_Data'high-i) <= rx_Nr_of_Data(i);  
			com_Int_nr_of_Data(com_rx_Nr_of_Data'high-i)  <= i_Int_nr_of_Data(i);
		end loop;
	end process;
  -----------------------------------------------------------------------------
  -- rx interrupt
  -----------------------------------------------------------------------------
    time_out   <= TimeOutCnt(8);
    timeout_and_int : process (Reset, Clk) is
    begin  -- process timeout
        if (Reset = '1') then
            TimeOutCnt <= "000000000";
            o_rxInt <= '0'; 
            buffer_Int <= '0';
            ...
            if Data_Present='0' then
                o_rxInt <= '0';    
            elsif (buffer_Int='1') or (time_out='1') then
                o_rxInt <= '1';
            end if;   
            -- buffer
            if (com_rx_Nr_of_Data >= com_Int_nr_of_Data) then 
                buffer_Int <= '1';
            else    
                buffer_Int <= '0';
            end if;
    ...
    if Reset = '1' then                 -- asynchronous reset (active high)
      ...
      if (EN_16x_Baud = '1') then
        if (stop_Bit_Position = '0') then
          ...
          --stop_Bit_Position <= sample_Point and data(STOP_BIT_POS);
          case i_data_bits is
          	when "00" =>	
          		if i_use_parity='1' then 
          			stop_Bit_Position <= sample_Point and data(STOP_BIT_POS-3);
          		else
          			stop_Bit_Position <= sample_Point and data(STOP_BIT_POS-4);
          		end if;			
          	when "01" =>	
          		if i_use_parity='1' then 
          			stop_Bit_Position <= sample_Point and data(STOP_BIT_POS-2);
          		else
          			stop_Bit_Position <= sample_Point and data(STOP_BIT_POS-3);
          		end if;
          	when "10" =>	
          		if i_use_parity='1' then 
          			stop_Bit_Position <= sample_Point and data(STOP_BIT_POS-1);
          		else
          			stop_Bit_Position <= sample_Point and data(STOP_BIT_POS-2);
          		end if;
          	when "11" =>	
      ...
      RX_Parity_Error <= '0';
    	...
    	if (EN_16x_Baud = '1') then
    		if mid_Start_Bit='1' then
    			...
    			if sample_Point='1' and stop_Bit_Position='0' then
    				-- build parity
    				Parity <= parity xor rx_2;
    			elsif sample_Point='1' and  stop_Bit_Position='0' then  
    				-- compare parity
    				RX_Parity_Error <= parity xor rx_2;	
    			end if;  
  ...
  --
  --  RX_Parity_Error <= (EN_16x_Baud and sample_Point) and (data(CALC_PAR_POS)) and
  --                     not stop_Bit_Position
  --                     when running and (rx_2 /= Parity) and i_use_parity='1' else '0';
  ...
  data(0) <= rx_2;      
  fifo_Din <= data(1 to C_DATA_BITS) when i_use_parity='0' else 
  			  data(2 to C_DATA_BITS+1); 
  Serial_To_Parallel : for I in 1 to serial_to_Par'length generate

    serial_to_Par(I) <= data(I) when (stop_Bit_Position or not sample_Point) = '1'
                        else data(I-1);
  ...
  SRL_FIFO_I : SRL_FIFO
    ...
    port map (
      ...
      Data_In     => fifo_Din,  -- [in  std_logic_vector(0 to C_DATA_BITS-1)]
      ...
      Nr_of_Data  => rx_Nr_of_Data,     -- [out std_logic_vector(0 to 3)]
      Data_Exists => Data_Present);  -- [out std_logic]
