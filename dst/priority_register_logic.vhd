-------------------------------------------------------------------------------
-- $Id: priority_register_logic.vhd,v 1.2 2003/03/31 19:15:36 anitas Exp $
-------------------------------------------------------------------------------
...
begin
    ...
    MASTER_LOOP:    for i in 0 to C_NUM_MASTERS-1 generate
        ...
        DECODE_GRANTS_PROCESS: process (mgrant_n_pad, priority_ids_i)
            variable j  : integer   := 0;
            
            begin
                for j in 0 to NUM_MSTRS_PAD-1 loop
                    if priority_ids_i(i*C_NUM_MID_BITS to i*C_NUM_MID_BITS + C_NUM_MID_BITS-1) = j then
                        grant_lvl_n(i) <= mgrant_n_pad(j);
                    else null;
                    end if;
                end loop;
            end process DECODE_GRANTS_PROCESS;
