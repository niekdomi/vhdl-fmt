...
--	29.01.2007-le	- add support vor virtex-4 and ddr-sdram
--	12.02.2008-le	- plb_bram: add cl support and sec. map (addr + 0x8000000)
--					- plb_ddr: add shadow ram map at 0xffb00000
--	25.02.2008-le	- correct ddr-ram C_DDR_COL_AWIDTH to 9 for ppc-card4 (ram: K4H561638H)
--	26.05.2008-le	- change ppc405_virtex4_v1_01_a C_DETERMINISTIC_MULT to 0 for better timing
--	08.09.2008-le	- remove generics for virtex2/virtex4 and sdram/ddr and 
--						split it to dif. files ppc_module_v2.vhd/ppc_module_v4.vhd
...
--	16.07.2010-le	- add generic C_SDRAM_SIZE
--------------------------------------------------------------------------------
--$Rev:: 1470                                                                  $
--$Author:: leuthold                                                           $
--$Date:: 2010-07-16 15:08:15 +0200 (Fr., 16 Jul 2010)                         $
--------------------------------------------------------------------------------
...
entity ppc_module is
	generic (
		...
		C_BASEADDR_PLBBRAM_IF_4 : std_logic_vector := X"7400_0000";
		C_HIGHADDR_PLBBRAM_IF_1 : std_logic_vector := X"70FF_FFFF";
		C_HIGHADDR_PLBBRAM_IF_2 : std_logic_vector := X"71FF_FFFF";
		C_HIGHADDR_PLBBRAM_IF_3 : std_logic_vector := X"73FF_FFFF";
		C_HIGHADDR_PLBBRAM_IF_4 : std_logic_vector := X"77FF_FFFF";
	...
	port (
		...
		i_plb_clk180 : in std_logic;
		i_plb_clk90 : in std_logic;
		i_plb_clk270 : in std_logic;
		i_ddr_clk90 : in std_logic;
		i_ddr_clk270 : in std_logic;
		...
		b_ddrDqs_i : in std_logic_vector(C_SDRAM_WIDTH/8-1 downto 0) := (others => L);
		b_ddrDq_i : in std_logic_vector(C_SDRAM_WIDTH-1 downto 0) := (others => L);
		b_ddrDqs_o : out std_logic_vector(C_SDRAM_WIDTH/8-1 downto 0);
		b_ddrDq_o : out std_logic_vector(C_SDRAM_WIDTH-1 downto 0);
		b_ddrDqs_t : out std_logic_vector(C_SDRAM_WIDTH/8-1 downto 0);
		b_ddrDq_t : out std_logic_vector(C_SDRAM_WIDTH-1 downto 0);
		-- sio
...
architecture STRUCTURE of ppc_module is
	...
	constant C_FAMILY : string := "virtex4";
	...
	signal ddrClkp : std_logic_vector(0 to 0) := (others => L);
	signal ddrClkn : std_logic_vector(0 to 0) := (others => L);
	signal ddrCe : std_logic_vector(0 to 1) := (others => L);
	signal ddrCsn : std_logic_vector(0 to 1) := (others => H);
	signal plb_clk180 : std_logic := L;
	signal plb_clk90 : std_logic := L;
	signal plb_clk270 : std_logic := L;
	signal ddr_clk90 : std_logic := L;
	signal ddr_clk270 : std_logic := L;
	signal ddrAddr : std_logic_vector(12 downto 0) := (others => L);
	signal ddrBankAddr : std_logic_vector(1 downto 0) := (others => L);
	signal ddrDm : std_logic_vector(1 downto 0) := (others => L);
	signal ddrDqs_i : std_logic_vector(1 downto 0) := (others => L);
	signal ddrDq_i : std_logic_vector(15 downto 0) := (others => L);
	signal ddrDqs_o : std_logic_vector(1 downto 0) := (others => L);
	signal ddrDq_o : std_logic_vector(15 downto 0) := (others => L);
	signal ddrDqs_t : std_logic_vector(1 downto 0) := (others => L);
	signal ddrDq_t : std_logic_vector(15 downto 0) := (others => L);
	signal ddrRASn : std_logic := H;
	signal ddrCASn : std_logic := H;
	signal ddrWEn : std_logic := H;
	...
	function coladdr(sdram_size : integer) return integer is
	begin
		if (sdram_size=64) then -- 64 mb 16bit ddr
			return 10;
		elsif (sdram_size=32)then -- 32 mb 16bit ddr
			return 9;
		else
			assert 1<0
			report "unsupported sdram size" severity error;
		end if;
	end coladdr;
...
begin
	...
	ppc405_sys : entity ppc405_virtex4_v1_01_a.ppc405_virtex4
	generic map (
		C_IDCR_BASEADDR => B"0000010000",	-- ISINIT:0x10, ISFILL 0x11, ISARC 0x12, ISCNTL 0x13
		C_IDCR_HIGHADDR => B"0000010011",	-- unused
	...
	port map (
		...
		BRAMISOCMDCRRDDBUS => net_gnd64(0 to 31),
		...
		DCREMACWRITE => open,
		EMACDCRACK => net_gnd0,
		EMACDCRDBUS => net_gnd64(0 to 31),
		EXTDCRABUS => open,
		EXTDCRDBUSOUT => open,
		EXTDCRREAD => open,
		EXTDCRWRITE => open,
		EXTDCRACK => net_gnd0,
		EXTDCRDBUSIN => net_gnd64(0 to 31),
...
begin
	...
	generic map (
		C_DEVICE => "2vp4"-- for ppc core count: 2vp4 and 4vfx12 have one
	)
	port map (
		TRSTNEG => net_vcc0,
	...
	-- inputs
	ddrDqs_i <= crossbits(b_ddrDqs_i);
	ddrDq_i <= crossbits(b_ddrDq_i);
	plb_clk180 <= i_plb_clk180;
	plb_clk90 <= i_plb_clk90;
	plb_clk270 <= i_plb_clk270;
	ddr_clk90 <= i_ddr_clk90;
	ddr_clk270 <= i_ddr_clk270;
	-- outputs
	o_ddrAddr <= crossbits(ddrAddr);
	o_ddrBankAddr <= crossbits(ddrBankAddr);
	o_ddrDm <= crossbits(ddrDm);
	b_ddrDqs_o <= crossbits(ddrDqs_o);
	b_ddrDq_o <= crossbits(ddrDq_o);
	b_ddrDqs_t <= crossbits(ddrDqs_t);
	b_ddrDq_t <= crossbits(ddrDq_t);
	no_ddrRas <= ddrRASn;
	no_ddrCas <= ddrCASn;
	no_ddrWe <= ddrWEn;
	o_ddrClkp <= ddrClkp(0);
	...
	plb_ddrsdram_sys : entity plb_ddr_v2_00_a.plb_ddr
	generic map (
		C_EXTRA_TSU => 0,
		...
		C_DDR_TRP => 20000,
		C_DDR_CAS_LAT => 2,
		C_DDR_DWIDTH => C_SDRAM_WIDTH,
		C_DDR_AWIDTH => 13,
		C_DDR_COL_AWIDTH => coladdr(C_SDRAM_SIZE),
		C_DDR_BANK_AWIDTH => 2,
		C_MEM0_BASEADDR => X"00000000",
		C_MEM0_HIGHADDR => X"03ffffff",
		C_MEM1_BASEADDR => X"ffb00000",
		C_MEM1_HIGHADDR => X"ffbfffff",
	...
	port map (
		PLB_Clk => plb_clk,
		PLB_Clk_n => plb_clk180,
		Clk90_in => plb_clk90,
		Clk90_in_n => plb_clk270,
		DDR_Clk90_in => ddr_clk90,
		DDR_Clk90_in_n => ddr_clk270,
		PLB_Rst => plb_PLB_Rst,
		PLB_ABus => plb_PLB_ABus,
		PLB_PAValid => plb_PLB_PAValid,
		PLB_SAValid => plb_PLB_SAValid,
