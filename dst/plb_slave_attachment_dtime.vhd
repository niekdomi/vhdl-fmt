-------------------------------------------------------------------------------
-- $Id: plb_slave_attachment_dtime.vhd,v 1.5 2004/11/23 01:10:26 jcanaris Exp $
-------------------------------------------------------------------------------
...
--
-------------------------------------------------------------------------------
-- Filename:        plb_slave_attachment_dtime.vhd
-- Version:         v1_00_a
-- Description:     PLB slave attachment supporting single beat transfers,
--                  cache line, and fixed length bursts. Design supports high
--                  speed data transfer (1 clock per data beat) on the
--                  cacheline and burst transfers.
...
use proc_common_v1_00_b.srl_fifo2;
use proc_common_v1_00_b.counter;
...
-------------------------------------------------------------------------------
entity plb_slave_attachment_dtime is
    generic (
        C_REG_CS_CE       : Boolean := true;
        ...
        C_ARD_DTIME_READ_ARRAY   : INTEGER_ARRAY_TYPE :=
           ...
           (
            0,0,1, -- IPIF Interrupt Determinate Read Params
            0,0,1, -- IPIF Reset Determinate Read Params
            0,0,1, -- IPIF WrFIFO Registers Determinate Read Params
            0,0,1, -- IPIF WrFIFO Data data Determinate Read Params
            0,0,1, -- IPIF RdFIFO Registers Determinate Read Params
            0,0,1, -- IPIF RdFIFO Data Determinate Read Params
            0,0,1, -- IP Registers Determinate Read Params
            1,0,1, -- User0 Determinate Read Params
            1,1,2  -- User1 Determinate Read Params
           );
        ...
        C_ARD_DTIME_WRITE_ARRAY   : INTEGER_ARRAY_TYPE :=
           ...
           (
            ...
            0,0,0, -- IP Registers Determinate Write Params
...
end entity plb_slave_attachment_dtime;

-------------------------------------------------------------------------------

-------------------------------------------------------------------------------

architecture implementation of plb_slave_attachment_dtime is
...
-------------------------------------------------------------------------------
 ...
 REG_PLB_INPUTS : Process (Bus_clk)
   begin
    ...
    I_DECODER : entity plb_ipif_v1_00_d.plb_address_decoder
    generic map(
   ...
   -- detect a command execute condition and set a flag if it exists
    ...
    --
    -- Note:
    -- Rearbitrates are initiated when the target asserts the IP2Bus_Retry
    -- signal prior to address phase completion. IP2Bus_Retry is used as a 'Busy'
    -- indication and is not part of a data phase reply as it is on the OPB.
    ------------------------------------------------------------------------------
    ...
    ------------------------------------------------------------
     INCLUDE_DATA_PHASE_WDT : if (C_DPHASE_TIMEOUT > 0) generate
       ...
       begin
        ...
        I_DPTO_COUNTER : entity proc_common_v1_00_b.Counter
          generic map(
 ...
 GEN_FAST_MODE_BURSTXFER : if (FAST_MODE_BURSTXFER) generate
   ...
   -- Component Declarations for Burst support
    ...
    begin
     ...
     -- Process Description:
     --     This process is used only during simulation to generate
     -- user warnings relating to burst inclusion.
     --
     -------------------------------------------------------------
     REPORT_BURST_WARNINGS : process (bus_clk)

        Variable newline : Character := cr;
        Variable indet_burst_warn : boolean;
        Variable report_inhibit_cnt : Integer := 5; -- 5 Bus_Clk clocks
          ...
          if (Bus_clk'event and Bus_clk = '1') then
             ...
             if (Bus_Reset = '1' or
               ...
               null; -- do nothing
                 ...
                 if (indeterminate_burst = '1' and Sl_AddrAck_i = '1') then
                    indet_burst_warn := true;
                 else
                    indet_burst_warn := false;
                 end if;


                Assert indet_burst_warn = false
                Report "PLB IPIF indetirminate burst transfer detected....  Only first data beat will complete!"
                Severity note;
           ...
           When PBRD_IDLE =>
              ...
              If (start_data_phase = '1' and
                 ...
                 If (indeterminate_burst = '1') Then  -- indeterminate burst request
                   ...
                   sl_rdbterm_ns           <= '1'; -- Terminate on first xfer
   ...
   -------------------------------------------------------------------------------
     ...
     WR_DATA_BUFFER : entity proc_common_v1_00_b.srl_fifo2
       generic map(
          ...
          Case ipif_wr_cntl_state Is
           ...
           When IWR_SINGLE1 =>
               ...
               if (Response_Ack_i = '1') then
                  ...
                  --ipif_wr_cntl_state_ns <= IWR_DONE;
                  ipif_wr_cntl_state_ns <= IWR_IDLE;
                  --clear_sl_wr_busy_ns   <= '1';
                  clear_sl_wr_busy      <= '1';
                  clear_wr_ce           <= '1';
   ...
   DO_WR_DATA_REG : process (bus_clk)
    ...
    WBUF_CYCLE_COUNTER : process (Bus_clk)
      ...
      ------------------------------------------------------------------------------
       CHECK_DATA_DONE : process (data_cycle_count, plb_side_dack)
         Begin
    ...
    GEN_DATA_XFER : process (rd_dphase_active,
     ...
     End process READ_DATA_REGISTER; --
      ...
      -- Determinate Timing Module instantiation
      I_DTIME_CONTROLLER : entity plb_ipif_v1_00_d.determinate_timer
        generic map(
          -- Generics
          C_NUM_CS                 =>  CS_BUS_SIZE,
          C_MAX_CYCLE_CNT          =>  MAX_FIXED_XFER_COUNT,
          C_ARD_DTIME_READ_ARRAY   =>  C_ARD_DTIME_READ_ARRAY,
          C_ARD_DTIME_WRITE_ARRAY  =>  C_ARD_DTIME_WRITE_ARRAY
          )
        port map(
          -- Input ports
          Bus_reset           =>  Bus_Reset,
          ...
          Req_Active          =>  data_request_active,
         -- num_data_beats      =>  num_data_beats,
          num_data_beats      =>  num_data_beats_minus1,
          Target_ReqAck       =>  data_ack,
     ...
     I_BUS_ADDRESS_COUNTER : entity plb_ipif_v1_00_d.addr_reg_cntr_brst_flex
      Generic map (
     ...
     I_STEER_ADDRESS_COUNTER : entity plb_ipif_v1_00_d.addr_reg_cntr_brst_flex
      Generic map (
 ...
 GEN_SLOW_MODE_BURSTXFER : if (SLOW_MODE_BURSTXFER) generate
    ...
    begin
       ...
       -- synthesis translate_off
         ...
         -- Process Description:
         --     This process is used only during simulation to generate
         -- user warnings relating to attempted burst transfers.
         --
         -------------------------------------------------------------
         REPORT_BURST_WARNINGS : process (bus_clk)

            Variable newline : Character := cr;
            Variable burst_warn : boolean;
            Variable report_inhibit_cnt : Integer := 5; -- 5 Bus_Clk clocks
              ...
              if (Bus_clk'event and Bus_clk = '1') then
                 ...
                 if (Bus_Reset = '1' or
                   ...
                   null; -- do nothing
                    ...
                    Assert burst_warn = false
                    Report "PLB IPIF indeterminate burst transfer detected....  Only first data beat will complete!" &
                    "    Indeterminate bursts are not supported by the PLB IPIF!"
                    Severity note;
          ...
          Case data_cntl_state  Is
            ...
            When IDLE =>
               ...
               --            sig_cmd_abort    = '0') Then
                  ...
                  -- If (plb_rnw_reg = '0') Then  -- write op
                  --
                  --   data_cntl_state_ns   <= WR_XFER1;
                  --   decode_ld_rw_ce      <= '1';
                  --   bus2ip_wrreq_ns      <= '1';
                  --   sl_wrbterm_ns        <= burst_transfer;
                  --
                  -- Else                         -- read op
                  --
                  --   data_cntl_state_ns    <= RD_XFER;
    ...
    GEN_DATA_XFER : process (data_cntl_state)
       begin
         ...
         if (data_cntl_state = RD_SINGLE or
             data_cntl_state = RD_CL_BRST1) then
           read_data_xfer <= '1';
         else
           read_data_xfer <= '0';
    ...
    -------------------------------------------------------------
     ...
     -- Main IPIF Address counter instantiation
     I_ADDRESS_COUNTER : entity plb_ipif_v1_00_d.addr_reg_cntr_brst_flex
      Generic map (
               C_NUM_ADDR_BITS   => C_PLB_ABUS_WIDTH,
               C_PLB_DWIDTH      => C_PLB_DBUS_WIDTH
              )
