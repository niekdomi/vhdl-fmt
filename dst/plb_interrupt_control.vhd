-------------------------------------------------------------------------------
-- $Id: plb_interrupt_control.vhd,v 1.1 2004/03/30 21:19:41 dougt Exp $
-------------------------------------------------------------------------------
--plb_interrupt_control.vhd   version v1.00b
-------------------------------------------------------------------------------
--
--  ***************************************************************************
...
--
-------------------------------------------------------------------------------
-- Filename:        plb_interrupt_control.vhd
--
...
entity plb_interrupt_control is
   Generic(
      C_NUM_CE              : INTEGER := 8;
      ...
      C_INCLUDE_DEV_PENCODER : BOOLEAN := false;-- Specifies device Priority Encoder function
      
      C_INCLUDE_DEV_ISC      : Boolean := false; -- Specifies device ISC hierarchy
   ...
   port(
      ...
      Bus2IP_Data_sa      : In  std_logic_vector(0 to C_IPIF_DWIDTH-1);
      Bus2IP_RdReq_sa     : In  std_logic;
      Bus2IP_WrReq_sa     : In  std_logic;
      Bus2IP_BE_sa        : In  std_logic_vector(0 to (C_IPIF_DWIDTH/8)-1);
  ...
  end plb_interrupt_control;

-------------------------------------------------------------------------------

architecture implementation of plb_interrupt_control is
  ...
  -- figure out if 32 bits wide or 64 bits wide
   ...
   -- Chip Enable Selection mapping (applies to RdCE and WrCE inputs)
    Constant DEVICE_ISR            : integer  := 0 /CHIP_SEL_SCALE_FACTOR;  --   0 if 64-bit dwidth;
    Constant DEVICE_IPR            : integer  := 1 /CHIP_SEL_SCALE_FACTOR;  --   0 if 64-bit dwidth;
    Constant DEVICE_IER            : integer  := 2 /CHIP_SEL_SCALE_FACTOR;  --   1 if 64-bit dwidth;
    Constant DEVICE_IAR            : integer  := 3 /CHIP_SEL_SCALE_FACTOR;  --   1 if 64-bit dwidth;
    Constant DEVICE_SIE            : integer  := 4 /CHIP_SEL_SCALE_FACTOR;  --   2 if 64-bit dwidth;
    Constant DEVICE_CIE            : integer  := 5 /CHIP_SEL_SCALE_FACTOR;  --   2 if 64-bit dwidth;
    Constant DEVICE_IIR            : integer  := 6 /CHIP_SEL_SCALE_FACTOR;  --   3 if 64-bit dwidth;
    Constant DEVICE_GIE            : integer  := 7 /CHIP_SEL_SCALE_FACTOR;  --   3 if 64-bit dwidth;
    Constant IP_ISR                : integer  := 8 /CHIP_SEL_SCALE_FACTOR;  --   4 if 64-bit dwidth;
    Constant IP_IPR                : integer  := 9 /CHIP_SEL_SCALE_FACTOR;  --   4 if 64-bit dwidth;
    Constant IP_IER                : integer  := 10/CHIP_SEL_SCALE_FACTOR;  --   5 if 64-bit dwidth;
    Constant IP_IAR                : integer  := 11/CHIP_SEL_SCALE_FACTOR;  --   5 if 64-bit dwidth;
    Constant IP_SIE                : integer  := 12/CHIP_SEL_SCALE_FACTOR;  --   6 if 64-bit dwidth;
    Constant IP_CIE                : integer  := 13/CHIP_SEL_SCALE_FACTOR;  --   6 if 64-bit dwidth;
    Constant IP_IIR                : integer  := 14/CHIP_SEL_SCALE_FACTOR;  --   7 if 64-bit dwidth;
    Constant IP_GIE                : integer  := 15/CHIP_SEL_SCALE_FACTOR;  --   7 if 64-bit dwidth;
    
   -- Column Selection mapping (applies to RdCE and WrCE inputs)
    Constant DEVICE_ISR_COL        : integer  := 0;
    Constant DEVICE_IPR_COL        : integer  := LSB_BYTLE_LANE_COL_OFFSET;
    Constant DEVICE_IER_COL        : integer  := 0;
    Constant DEVICE_IAR_COL        : integer  := LSB_BYTLE_LANE_COL_OFFSET;
    Constant DEVICE_SIE_COL        : integer  := 0;
    Constant DEVICE_CIE_COL        : integer  := LSB_BYTLE_LANE_COL_OFFSET;
    Constant DEVICE_IIR_COL        : integer  := 0;
    Constant DEVICE_GIE_COL        : integer  := LSB_BYTLE_LANE_COL_OFFSET;
    Constant IP_ISR_COL            : integer  := 0;
    Constant IP_IPR_COL            : integer  := LSB_BYTLE_LANE_COL_OFFSET;
    Constant IP_IER_COL            : integer  := 0;
    Constant IP_IAR_COL            : integer  := LSB_BYTLE_LANE_COL_OFFSET;
    Constant IP_SIE_COL            : integer  := 0;
    Constant IP_CIE_COL            : integer  := LSB_BYTLE_LANE_COL_OFFSET;
    Constant IP_IIR_COL            : integer  := 0;
    Constant IP_GIE_COL            : integer  := LSB_BYTLE_LANE_COL_OFFSET;
 ...
 --INTERNAL SIGNALS
    ...
    signal  irpt_read_data             : std_logic_vector(DBUS_WIDTH_MINUS1 downto 0);
    ...
    Signal  column_sel                 : std_logic_vector(0 to LSB_BYTLE_LANE_COL_OFFSET);
...
begin
 ...
 -- Misc I/O and Signal assignments 
    ...
    --
    -------------------------------------------------------------
    ASSIGN_COL : process (Bus2IP_BE_sa)
       begin
    
         -- Assign the 32-bit column selects from BE inputs
         for i in 0 to LSB_BYTLE_LANE_COL_OFFSET loop
           column_sel(i)    <= Bus2IP_BE_sa(i*4);
         end loop;
        
       end process ASSIGN_COL; 
    
    --column_sel(0)    <= Bus2IP_BE_sa(0);
    --column_sel(1)    <= Bus2IP_BE_sa(4);    
            ...
            if (Bus2IP_Clk_i'event and Bus2IP_Clk_i = '1') Then
               ...
               If (Bus2IP_Reset_i = '1') Then
                  ...
                  ip_irpt_status_reg(irpt_index) <= '0'; 
                      ...
                      column_sel(IP_ISR_COL) = '1') Then -- toggle selected ISR bits from the DBus inputs
          
                  ip_irpt_status_reg(irpt_index) <= 
                   (Bus2IP_Data_sa(DBUS_WIDTH_MINUS1-irpt_index-32) xor  -- toggle bits on write of '1'
       ...
       if (Bus2IP_Clk_i'event and Bus2IP_Clk_i = '1') Then
         ...
         If (Bus2IP_Reset_i = '1') Then
            ...
            ip_irpt_enable_reg <= (others => '0'); 
                ...
                column_sel(DEVICE_IER_COL) = '1') Then -- load input data from the DBus inputs

            ip_irpt_enable_reg <= Bus2IP_Data_sa(DBUS_WIDTH_MINUS1-IP_IRPT_HIGH_INDEX-32 
                                                 to DBUS_WIDTH_MINUS1-32);
       ...
       if (Bus2IP_Clk_i'event and Bus2IP_Clk_i = '1') Then
         ...
         If (Bus2IP_Reset_i = '1') Then
            ...
            ipif_irpt_status_reg <= (others => '0'); 
                ...
                column_sel(DEVICE_ISR_COL) = '1') Then -- load input data from the DBus inputs

            for i in 0 to 1 loop                                                            
              ipif_irpt_status_reg(i) <= (Bus2IP_Data_sa(DBUS_WIDTH_MINUS1-i-32) xor  -- toggle bits on write of '1'     
    ...
    Begin
       ...
       if (Bus2IP_Clk_i'event and Bus2IP_Clk_i = '1') Then
          ...
          If (Bus2IP_Reset_i = '1') Then
             ...
             ipif_irpt_enable_reg <= Bus2IP_Data_sa(DBUS_WIDTH_MINUS1-IPIF_IRPT_HIGH_INDEX-32  to 
                                                    DBUS_WIDTH_MINUS1-32);
      ...
      if (Bus2IP_Clk_i'event and Bus2IP_Clk_i = '1') Then
        ...
        If (Bus2IP_Reset_i = '1') Then
           ...
           ipif_glbl_irpt_enable_reg <= '0'; 
               ...
               column_sel(DEVICE_GIE_COL) = '1') Then -- load input data from the DBus inputs
 
           ipif_glbl_irpt_enable_reg <= Bus2IP_Data_sa(DBUS_WIDTH_MINUS1-31);
