-------------------------------------------------------------------------------
-- $Id: arbitration_logic.vhd,v 1.2 2003/03/31 19:15:36 anitas Exp $
------------------------------------------------------------------------------
...
MASTERLOOP: for i in 0 to C_NUM_MASTERS-1 generate
       ...
       DECODE_REQ_PROCESS: process(m_request_n, Priority_ids)
       
       begin
            for j in 0 to NUM_MSTRS_PAD-1 loop
                if Priority_ids(i*C_NUM_MID_BITS to i*C_NUM_MID_BITS+C_NUM_MID_BITS-1) = j then
                    request_lvl_n(i) <= m_request_n(j);
                else
                    null;
                end if;
            end loop;
