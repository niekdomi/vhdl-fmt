...
TEST_CASE("Doc System", "[doc]")
{
    ...
    SECTION("Operators")
    {
        ...
        {
            SECTION("Direct concatenation (+)") { REQUIRE((a + b).render(defaultConfig()) == "ab"); }
            SECTION("Space concatenation (&)")  { REQUIRE((a & b).render(defaultConfig()) == "a b"); }
            SECTION("Soft line concatenation (/)") { REQUIRE((a / b).render(defaultConfig()) == "a\nb"); }
            SECTION("Hard line concatenation (|)") { REQUIRE((a | b).render(defaultConfig()) == "a\nb"); }
        ...
        SECTION("Compound Assignment Operators")
        {
            ...
            SECTION("Nest Append (<<=)")
            {
                Doc doc = Doc::text("root"); doc <<= Doc::text("child");
    ...
    SECTION("Alignment")
    {
        ...
        {
            // Regression Test: Ensure keywords don't become plain text after padding
            config.casing.keywords = common::CaseStyle::UPPER;

            // "in" should act as a keyword (UPPER) AND be aligned
            // "out" should act as a keyword (UPPER) AND be aligned
            const Doc row1 = Doc::keyword("in", 1);
            const Doc row2 = Doc::keyword("out", 1);

            const Doc doc = Doc::align(row1 / row2);

            // "out" is length 3. "in" is length 2.
            // "in" needs 1 space padding.
            // Expected: "IN \nOUT"
