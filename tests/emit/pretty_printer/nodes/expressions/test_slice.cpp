...
TEST_CASE("SliceExpr Rendering", "[pretty_printer][expressions][slice]")
{
    SECTION("Simple slice with downto")
    {
        ast::BinaryExpr range{
          .left{std::make_unique<ast::Expr>(ast::TokenExpr{.text{"7"}})},
          .op{"downto"},
          .right{std::make_unique<ast::Expr>(ast::TokenExpr{.text{"0"}})},
        };

        const ast::SliceExpr slice{
          .prefix{std::make_unique<ast::Expr>(ast::TokenExpr{.text{"data"}})},
    ...
    {
        ast::BinaryExpr range{
          .left{std::make_unique<ast::Expr>(ast::TokenExpr{.text{"0"}})},
          .op{"to"},
          .right{std::make_unique<ast::Expr>(ast::TokenExpr{.text{"7"}})},
        };
