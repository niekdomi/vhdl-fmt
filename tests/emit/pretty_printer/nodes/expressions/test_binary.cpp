...
TEST_CASE("BinaryExpr Rendering", "[pretty_printer][expressions][binary]")
{
    ...
    {
        ast::BinaryExpr inner{.left{std::make_unique<ast::Expr>(ast::TokenExpr{.text{"a"}})},
                              .op{"+"},
                              .right{std::make_unique<ast::Expr>(ast::TokenExpr{.text{"b"}})}};
    ...
    {
        ast::BinaryExpr mult{.left{std::make_unique<ast::Expr>(ast::TokenExpr{.text{"b"}})},
                             .op{"*"},
                             .right{std::make_unique<ast::Expr>(ast::TokenExpr{.text{"c"}})}};

        const ast::BinaryExpr add{.left{std::make_unique<ast::Expr>(ast::TokenExpr{.text{"a"}})},
                                  .op{"+"},
