...
TEST_CASE("TypeDecl: Array", "[pretty_printer][type][array]")
{
    ...
    SECTION("Array with element constraint")
    {
        ...
        ast::BinaryExpr arr_range{
          .left = std::move(arr_left), .op = "to", .right = std::move(arr_right)};
        ...
        ast::BinaryExpr elem_range{
          .left = std::move(elem_left), .op = "downto", .right = std::move(elem_right)};
