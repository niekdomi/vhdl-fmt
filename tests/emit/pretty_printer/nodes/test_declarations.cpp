...
TEST_CASE("GenericParam Rendering", "[pretty_printer][declarations]")
{
    ...
    param.subtype = ast::SubtypeIndication{
    ...
    SECTION("With Default Values")
    {
        ...
        SECTION("Multiple Names with Default")
        {
            param.names = {"A", "B"};
            param.subtype = ast::SubtypeIndication{
              .resolution_func = std::nullopt, .type_mark = "natural", .constraint = std::nullopt};
...
TEST_CASE("Port Rendering", "[pretty_printer][declarations]")
{
    ...
    // Default common settings
    port.mode = "in";
    port.subtype = ast::SubtypeIndication{
      .resolution_func = std::nullopt, .type_mark = "std_logic", .constraint = std::nullopt};
    ...
    SECTION("With Constraints")
    {
        ...
        SECTION("Multi-dimensional Range")
        {
            ...
            ast::BinaryExpr range1{.left = std::make_unique<ast::Expr>(ast::TokenExpr{.text = "7"}),
                                   .op = "downto",
                                   .right =
                                     std::make_unique<ast::Expr>(ast::TokenExpr{.text = "0"})};

            // Constraint 2: 3 downto 0
            ast::BinaryExpr range2{.left = std::make_unique<ast::Expr>(ast::TokenExpr{.text = "3"}),
                                   .op = "downto",
                                   .right =
                                     std::make_unique<ast::Expr>(ast::TokenExpr{.text = "0"})};
