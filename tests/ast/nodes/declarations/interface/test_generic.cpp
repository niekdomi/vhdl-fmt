...
TEST_CASE("Declaration: Generic Param", "[builder][decl][interface]")
{
    SECTION("Standard Generic")
    {
        ...
        const auto& gen = entity->generic_clause.generics.at(0);
        CHECK(gen.names.at(0) == "CLK_PERIOD");
    ...
    SECTION("Generic with default")
    {
        ...
        const auto& gen = generics.at(0);
        REQUIRE(gen.names.size() == 1);
        CHECK(gen.names.at(0) == "WIDTH");
    ...
    SECTION("Generic with multiple names (comma-separated)")
    {
        ...
        CHECK(gen.names.at(0) == "N");
        CHECK(gen.names.at(1) == "M");
        CHECK(gen.subtype.type_mark == "integer");
    }
    ...
    SECTION("Multiple generic declarations (semicolon-separated)")
    {
        ...
        // First declaration
        CHECK(generics.at(0).names.at(0) == "A");
        CHECK(generics.at(0).subtype.type_mark == "integer");

        // Second declaration
        CHECK(generics.at(1).names.at(0) == "B");
        CHECK(generics.at(1).subtype.type_mark == "boolean");
        REQUIRE(generics.at(1).default_expr.has_value());
    }
}
