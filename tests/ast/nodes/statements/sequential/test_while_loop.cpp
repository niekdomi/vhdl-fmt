...
TEST_CASE("WhileLoop", "[statements][while_loop]")
{
    ...
    SECTION("Simple while loop")
    {
        ...
        // Access Wrapper -> Kind
        const auto* assign = std::get_if<ast::VariableAssign>(&loop->body.at(0).kind);
    ...
    SECTION("Comparison condition")
    {
        ...
        CHECK(std::holds_alternative<ast::VariableAssign>(loop->body.at(1).kind));
    }
